...
void test_defaultConstructor() {
    my::function<int(void)> f(foo);
    assert(f() == 1);
}

void test_copyConstructor() {
    my::function<int(void)> b(bar);
    my::function<int(void)> second(b);
    assert(second() == 2);
}

void test_nullptrConstructor() {
    my::function<void(void)> f(nullptr);
}

void test_moveConstructor() {
    my::function<int(void)> b(bar);
    my::function<int(void)> second(std::move(b));
    assert(second() == 2);
}
...
void test_moveAssignment() {
    my::function<int(void)> b(bar);
    my::function<int(void)> second(foo);
    second = std::move(b);
    assert(second() == 2);
}

void test_explicitOperatorBool() {
    my::function<int(void)> f(nullptr);
    assert(!f);
    f = foo;
    assert(f);
}
...
void test_swap() {
    my::function<int()> f(foo);
    my::function<int()> b(bar);
    assert(f() == 1);
    assert(b() == 2);
...
void NIKITOZZZZ_test() {
    ...
    my::function<int (std::ostream &)> f([=](std::ostream &os) mutable {
